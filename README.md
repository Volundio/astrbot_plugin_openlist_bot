...
* 📤 **文件上传** - 回复图片、视频或文件消息后使用 `/ol upload` 直接上传，优先使用 URL 流式中转上传。
* 🔍 **文件搜索** - 支持在指定目录中搜索目标文件。
...
| `/ol upload` | `/网盘 上传` | `/ol upload /path` | 上传被引用消息中的图片、视频或文件 |
...
# 回复图片、视频或文件消息后，上传到当前目录
/ol upload

# 回复图片、视频或文件消息后，上传到指定目录
/ol upload /movies

# 回复图片、视频或文件消息后，上传到当前目录下的子目录
/ol upload clips
```
...
* `/ol upload` 需要回复一条包含图片、视频或文件的消息；插件会通过 OneBot 获取被引用消息并上传其中的媒体/文件。
