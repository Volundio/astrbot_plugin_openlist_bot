...
* 直接发送文件需要本地临时文件，不能完全像上传那样无本地文件中转。
* 上传优先使用平台提供的文件 URL 进行流式中转，失败时再回退到本地临时文件上传。
...
A: `/ol autobackup enable/disable` 需要群主或管理员权限。请在群聊中由群主或管理员执行。
